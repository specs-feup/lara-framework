...
public class BaseNodes {
    ...
    public static String toXml(Base base) {
        try {
            return StringUtils.xmlToStringBuffer(base.getXmlDocument(), 3).toString();
        } catch (TransformerFactoryConfigurationError | TransformerException e) {
            throw new RuntimeException("Could not convert aspect IR node to XML: ", e);
        }
    }

    public static Stream<Base> toStream(Base base) {
        return BaseVisitor.getBaseToStream().apply(base);
    }
