import { JSONtoFile, fileToJSON } from "../core/output.ts";
import JavaTypes, { type JavaClasses } from "./util/JavaTypes.ts";
...
export default class Io {
  ...
  /**
   * @deprecated Use JSONtoFile from api/core/output.ts instead
   */
  ...
  static copyFile(
    filepath: string | JavaClasses.File,
    destination: string | JavaClasses.File
  ) {
    if (filepath === undefined) throw new Error("Value filepath is undefined");
   ...
   *
   * @deprecated Use JSON.parse() instead or fileToJSON from api/core/output.ts
   */
