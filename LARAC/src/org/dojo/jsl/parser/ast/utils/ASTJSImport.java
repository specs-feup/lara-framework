...
public class ASTJSImport extends SimpleNode {

    private static final String NODE_NAME = "JSImport";

    private final List<String> qualifiedImport;

    public ASTJSImport(List<String> qualifiedImport) {
        super(-1);
    ...
    public void globalToXML(Document doc, Element parent) {
        ...
        statementDeclEl.setAttribute("desc", qualifiedImport.stream().collect(Collectors.joining("/", "", ".js")));

        // parent.appendChild(statementDeclEl);

        // Get first node that is not a JSImport, or null if there are no nodes or only has imports
    ...
    public String toString() {
        return "JSScript (" + qualifiedImport + ")";
    }
}
