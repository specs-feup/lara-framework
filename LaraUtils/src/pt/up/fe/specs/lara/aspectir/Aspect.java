...
public class Aspect extends Base implements IElement {
    ...
    public void visitDepthFirst(Visitor visitor) {
        if (check != null) {
            check.visitDepthFirst(visitor);
        }

        if (finalize != null) {
            finalize.visitDepthFirst(visitor);
        }

        if (initialize != null) {
            initialize.visitDepthFirst(visitor);
        }

        if (parameters != null) {
            parameters.visitDepthFirst(visitor);
        }

        statements.stream().forEach(stmt -> stmt.visitDepthFirst(visitor));

        if (staticBlock != null) {
            staticBlock.visitDepthFirst(visitor);
        }
